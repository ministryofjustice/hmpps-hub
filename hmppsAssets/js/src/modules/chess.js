export default (function () {
  return {
    init: function init() {
      const chess = document.querySelector('.js-chess');
      const btn = document.querySelector('.js-newgame');
      if (!chess) {
        return false;
      }
      ...
      const newGame = function newGame() {
        ...
        const skill = parseInt($('#skillLevel').val());
        game.reset();
    ...
    engineGame(options) {
      ...
      const evaler = typeof STOCKFISH === 'function' ? STOCKFISH() : new Worker(options.stockfishjs || 'stockfish.js');
      const engineStatus = {};
      ...
      const evaluation_el = document.getElementById('evaluation');
      ...
      function displayClock(color, t) {
        ...
        const id = color == playerColor ? '#time2' : '#time1';
      ...
      function clockTick() {
        updateClock();
        const t = (time.clockColor == 'white' ? time.wtime : time.btime) + time.startTime - Date.now();
      ...
      const onDrop = (source, target) => {
        ...
        const radios = document.querySelectorAll('.js-promote');
        let radio;
